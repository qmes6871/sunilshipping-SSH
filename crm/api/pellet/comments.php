...
// 테이블 존재 확인 및 생성
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS crm_pellet_activity_comments (
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT c.*, u.name as user_name
            FROM crm_pellet_activity_comments c
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'create':
            ...
            if ($hasImage) {
                $uploadResult = uploadFile($_FILES['image'], 'pellet/comments', CRM_ALLOWED_IMAGE_TYPES);
            ...
            $depth = 0;
            if ($parentId) {
                $stmt = $pdo->prepare("SELECT depth FROM crm_pellet_activity_comments WHERE id = ?");
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO crm_pellet_activity_comments
        ...
        case 'update':
            ...
            try {
                $stmt = $pdo->prepare("UPDATE crm_pellet_activity_comments
        ...
        case 'delete':
            ...
            try {
                $stmt = $pdo->prepare("DELETE FROM crm_pellet_activity_comments WHERE id = ?");
                $stmt->execute([$id]);
