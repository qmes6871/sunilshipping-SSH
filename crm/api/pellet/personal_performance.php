...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'create':
            ...
            $itemName = trim($input['item_name'] ?? $input['trade_type'] ?? '');
            ...
            try {
                ...
                error_log("Pellet personal performance create error: " . $e->getMessage());
        ...
        case 'delete':
            ...
            try {
                $stmt = $pdo->prepare("DELETE FROM " . CRM_PELLET_PERSONAL_PERFORMANCE_TABLE . " WHERE id = ?");
                $stmt->execute([$id]);
