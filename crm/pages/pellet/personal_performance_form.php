...
if ($editId) {
    try {
        $stmt = $pdo->prepare("SELECT * FROM " . CRM_PELLET_PERSONAL_PERFORMANCE_TABLE . " WHERE id = ?");
        $stmt->execute([$editId]);
        $editData = $stmt->fetch();
        if ($editData) {
            $isEdit = true;
...
$pageSubtitle = '우드펠렛 월별 개인 실적 ' . ($isEdit ? '수정' : '등록');
...
<div class="container">
    ...
    <p class="subtitle">우드펠렛 월별 개인 실적 <?= $isEdit ? '수정' : '등록' ?></p>
