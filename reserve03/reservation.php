...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        // 로그인한 사용자의 user_id 가져오기 (customer_management 테이블의 username)
        $user_id = null;
        if ($is_logged_in && !empty($_SESSION['username'])) {
            try {
                // customer_management 테이블에서 username 조회
                $stmt_user = $conn->prepare("
                    ...
                    LIMIT 1
                ");
                $stmt_user->execute([$_SESSION['username'], $_SESSION['username']]);
                $user_data = $stmt_user->fetch(PDO::FETCH_ASSOC);

                if ($user_data && !empty($user_data['username'])) {
                    $user_id = $user_data['username']; // customer_management의 username 저장
                } else {
                    // customer_management에 없으면 세션값 사용
        ...
        $stmt = $conn->prepare("
            ...
            (product_id, ship_id, username, name, email, phone, customer_name, customer_email,
...
<body>
    ...
    <div class="container">
        ...
        <div class="product-info">
            ...
            <div class="section-title" style="margin-bottom: 1rem;">화물 및 운송 정보</div>
            ...
            <div class="product-details">
                ...
                <div class="detail-item">
                    ...
                    <div class="detail-value"><?= !empty($product['transit_time']) ? $product['transit_time'] . '일' : '문의' ?></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">운송료</div>
                    <div class="detail-value"><?= !empty($product['price']) ? '$' . number_format($product['price'], 2) : '문의' ?></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">무게</div>
                    <div class="detail-value"><?= !empty($product['weight']) ? htmlspecialchars($product['weight']) : '문의' ?></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">화물 타입 (TYPE)</div>
                    <div class="detail-value"><?= !empty($product['cargo_type']) ? htmlspecialchars($product['cargo_type']) : '문의' ?></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">결제 조건 (결제TERM)</div>
                    <div class="detail-value"><?= !empty($product['payment_terms']) ? htmlspecialchars($product['payment_terms']) : '문의' ?></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">요율 (RATE)</div>
                    <div class="detail-value"><?= !empty($product['rate']) ? htmlspecialchars($product['rate']) : '문의' ?></div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">유효기간</div>
                    <div class="detail-value"><?= !empty($product['valid_until']) ? htmlspecialchars(preg_replace('/\s*\([^)]*\)/', '', $product['valid_until'])) : '문의' ?></div>
                </div>
                <div class="detail-item" style="grid-column: 1 / -1;">
                    <div class="detail-label">추가 정보 (기타)</div>
