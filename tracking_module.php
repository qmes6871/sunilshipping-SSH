...
.ct-hidden-row { display:none; }
.ct-load-more-btn { display:block; width:100%; padding:12px; margin-top:12px; background:#f8fafc; border:1px solid #e5e7eb; border-radius:8px; color:#374151; font-weight:600; cursor:pointer; transition:all 0.2s; }
.ct-load-more-btn:hover { background:#e5e7eb; }
...
// 세션에서 로그인 여부 먼저 확인
$sessionUsername = $_SESSION['ss_mb_id'] ?? $_SESSION['username'] ?? null;
$isGuest = ($sessionUsername === null || $sessionUsername === '');

// korean_name 조회 (로그인 사용자만)
$koreanName = null;
if (!$isGuest) {
    $koreanName = ct_get_logged_in_korean_name($pdo);
    // korean_name이 없으면 username을 대신 사용
    if ($koreanName === null || $koreanName === '') {
        $koreanName = $sessionUsername;
    }
}
...
if (!$isGuest) {
    ...
    if (empty($r1)) {
        ...
        echo '<div style="overflow-x:auto;"><table id="table1" class="ct-table"><thead><tr>'
           ...
           . '</tr></thead><tbody>';
        foreach ($r1 as $i => $row) {
            $hiddenClass = ($i >= 5) ? ' class="ct-hidden-row"' : '';
            echo '<tr' . $hiddenClass . '>'
               . '<td>' . ($i+1) . '</td>'
        ...
        if (count($r1) > 5) {
            echo '<button class="ct-load-more-btn" onclick="loadMoreRows(\'table1\', this)">More </button>';
        }
    }
}
...
if (!$isGuest) {
    ...
    if (empty($r2)) {
        ...
        echo '</tbody></table></div>';
        if (count($r2) > 5) {
            echo '<button class="ct-load-more-btn" onclick="loadMoreRows(\'table2\', this)">More</button>';
        }
    }
}
...
if (empty($r3)) {
    ...
    echo '</tbody></table></div>';
    if (count($r3) > 5) {
        echo '<button class="ct-load-more-btn" onclick="loadMoreRows(\'table3\', this)">More</button>';
    }
}
...
// JavaScript
echo '<script>
function loadMoreRows(tableId, btn) {
    const table = document.getElementById(tableId);
    if (!table) return;

    const hiddenRows = table.querySelectorAll("tbody tr.ct-hidden-row");
    let shown = 0;

    hiddenRows.forEach(row => {
        if (shown < 5) {
            row.classList.remove("ct-hidden-row");
            shown++;
        }
    });

    const remaining = table.querySelectorAll("tbody tr.ct-hidden-row").length;
    if (remaining === 0) {
