...
echo "<h2>Google Sheets API (서비스 계정) - 25'W/W 시트</h2>";
...
// 필요한 컬럼만 추출 (C, E, F, G, X, Y, Z, AA, AB, AC, AD, AE, AF, AG, AH, AI, BR)
// C=2, E=4, F=5, G=6, X=23, Y=24, Z=25, AA=26, AB=27, AC=28, AD=29, AE=30, AF=31, AG=32, AH=33, AI=34, BR=69
$columnIndexes = [2, 4, 5, 6, 23, 24, 25, 26, 69];
$columnNames = ['C', 'E', 'F', 'G', 'X', 'Y', 'Z', 'AA', 'BR'];
...
try {
    ...
    for ($idx = 1; $idx < count($filteredRows); $idx++) {
        ...
        $sql = "INSERT INTO ww_tracking (" . implode(', ', $columns) . ")
    ...
    // 총 레코드 수 확인
    $result = $conn->query("SELECT COUNT(*) as total FROM ww_tracking");
    $totalCount = 0;
    if ($result) {
